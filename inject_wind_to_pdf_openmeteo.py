...
Texte noir, aligné à droite, écriture ligne par ligne (fiable).
"""
...
def draw_right_aligned_text(page, x_right: float, y_top: float, lines, fontsize: float = 9.0, leading: float = 3.0, fontname: str = "helv"):
    """
    Écrit chaque ligne alignée à droite en mesurant la largeur du texte.
    x_right = bord droit interne du cartouche
    y_top   = bord haut interne du cartouche
    """
    color = (0, 0, 0)
    y = y_top + fontsize  # baseline de la 1ère ligne
    for txt in lines:
        try:
            width = page.get_text_length(txt, fontname=fontname, fontsize=fontsize)
            page.insert_text((x_right - width, y), txt, fontname=fontname, fontsize=fontsize, color=color)
        except Exception:
            # repli si la police "helv" n'est pas dispo : Times-Roman
            width = page.get_text_length(txt, fontname="Times-Roman", fontsize=fontsize)
            page.insert_text((x_right - width, y), txt, fontname="Times-Roman", fontsize=fontsize, color=color)
        y += fontsize + leading

def draw_cartouche(page, x: float, y: float, w: float, h: float, lines, fontsize: float = 9.0, fill=True):
    """Cadre fin gris, fond blanc (désactivable avec fill=False), texte à droite."""
    ...
    # écriture ligne par ligne alignée à droite
    draw_right_aligned_text(page, inner.x1, inner.y0, lines, fontsize=fontsize, leading=3.0, fontname="helv")
